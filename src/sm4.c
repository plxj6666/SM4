...
int sm4_make_dec_subkeys(const unsigned char key[SM4_KEY_SIZE], uint32_t decSubKeys[SM4_ROUNDS]) {
    // 解密子密钥就是加密子密钥的逆序
    sm4_make_enc_subkeys(key, decSubKeys);
    
    // 逆序排列
    uint32_t tmp;
    for(int i = 0; i < 16; i++) {
        tmp = decSubKeys[i];
        decSubKeys[i] = decSubKeys[31-i];
        decSubKeys[31-i] = tmp;
    }
...
void sm4_encrypt_block(const unsigned char *input, const uint32_t encSubKeys[SM4_ROUNDS], unsigned char *output) {
    ...
    uint32_t tmp;
    for(int i = 0; i < 32; i++) {
        tmp = F(X[0], X[1], X[2], X[3], encSubKeys[i]);
        X[0] = X[1];
        X[1] = X[2];
        X[2] = X[3];
        X[3] = tmp;
    ...
    // 反序变换
    for(int i = 0; i < 4; i++) {
        output[4*(3-i)] = (X[i] >> 24) & 0xFF;
        output[4*(3-i)+1] = (X[i] >> 16) & 0xFF;
        output[4*(3-i)+2] = (X[i] >> 8) & 0xFF;
        output[4*(3-i)+3] = X[i] & 0xFF;
    }
}
